...
export default function CheckoutPage() {
  ...
  return (
    <div dir="ltr" className="main-viewport" style={{ backgroundColor: 'var(--bg-secondary)', minHeight: '100vh', padding: isClosed ? '80px 20px 40px 20px' : '40px 20px' }}>
      {isClosed && (
                    ...
                    <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontSize: '0.85rem' }}>
                      <div>
                        ...
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                          ...
                          {item.addons?.length > 0 && `, +${item.addons.map(a => resolveText(a)).join(', ')}`}
                          {item.removedIngredients?.length > 0 && `, (${d.no || 'No'} ${item.removedIngredients.map(r => resolveText(r)).join(', ')})`}
                        </span>
                        {item.notes && (
