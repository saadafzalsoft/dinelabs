...
export async function createTranslationMap(text, languages = ['en', 'ar'], sourceLang = 'en') {
  if (!text) return { en: '' };
  
  // Pivot translation: if sourceLang is not English, translate it to English first
  let englishText = text;
  if (sourceLang !== 'en') {
    englishText = await translateText(text, 'en', sourceLang);
  }

  const map = { en: englishText };
  map[sourceLang] = text; // Keep source language as the exact original input

  for (const lang of languages) {
    if (lang === 'en' || lang === sourceLang) continue;
    map[lang] = await translateText(englishText, lang, 'en');
  }
  return map;
}
