...
export async function sendOrderNotifications(order, tenant) {
  if (!tenant || !tenant.notifications) {
    console.log('Notifications not configured for tenant');
    return;
  }
  ...
  if (notifications.emailEnabled && notifications.emailRecipient) {
    try {
  ...
  if (notifications.whatsappEnabled && notifications.whatsappRecipient) {
    try {
      ...
      if (!accountSid || !authToken) {
        ...
        params.append('To', `whatsapp:${notifications.whatsappRecipient}`);
